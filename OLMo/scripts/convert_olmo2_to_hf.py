...
from transformers.models.olmo2.configuration_olmo2 import DynamicPeMode
...
def write_model(
    ...
    dynamic_pe_mode = olmo2_config.get('dynamic_pe_mode', DynamicPeMode.N)
    dynamic_pe_start_layer = olmo2_config.get('dynamic_pe_start_layer', 0)
    dynamic_pe_layer_ids = olmo2_config.get('dynamic_pe_layer_ids', "")
    ...
    for layer_i in range(n_layers):
        ...
        if dynamic_pe_mode in [DynamicPeMode.SEXMH, DynamicPeMode.SEXMHPD]:
            if layer_i >= dynamic_pe_start_layer:
                state_dict[f"model.layers.{layer_i}.self_attn.gate_map.weight"] = loaded[
                    f"transformer.blocks.{layer_i}.gate_map.weight"
                ]
                state_dict[f"model.layers.{layer_i}.self_attn.content_map.weight"] = loaded[
                    f"transformer.blocks.{layer_i}.content_map.weight"
                ]
                state_dict[f"model.layers.{layer_i}.self_attn.final_map.weight"] = loaded[
                    f"transformer.blocks.{layer_i}.final_map.weight"
                ]
        elif dynamic_pe_mode == DynamicPeMode.SEXMHFEX:
            if layer_i >= dynamic_pe_start_layer:
                state_dict[f"model.layers.{layer_i}.self_attn.gate_map.weight"] = loaded[
                    f"transformer.blocks.{layer_i}.gate_map.weight"
                ]
                ...
                state_dict[f"model.layers.{layer_i}.self_attn.filter.weight"] = loaded[
                    f"transformer.blocks.{layer_i}.filter.weight"
                ]
        elif dynamic_pe_mode not in [DynamicPeMode.N, DynamicPeMode.NOPE]:
    ...
    pe_config = {
        ...
        'dynmaic_pe_start_layer': dynamic_pe_start_layer, # compatible with a type in config
        'dynamic_pe_start_layer': dynamic_pe_start_layer,
        'dynamic_pe_layer_ids': dynamic_pe_layer_ids,
        'dynmaic_pe_layer_ids': dynamic_pe_layer_ids
    }
