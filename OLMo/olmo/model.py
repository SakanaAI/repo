...
class DynamicPeMode:
    ...
    @staticmethod
    def verify(x):
        return x in ['none', 'multihead_continuous', 'continuous', 'discrete', 'continuous_with_scale_control', 'attention', 'swiglu', 'swigluex', 'nope', 'swiglu_scientific', 'swiglu_scientific_tanh', 'swigluex_posdim', 'swigluex_filterex', 'swigluex_posavg', 'swigluex_multihead', 'swigluex_sigmoid', 'swigluex_multihead_posdim', 'swigluex_multihead_filterex', 'cope']
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # dynamic pe
        dynamic_pe_mode = config.dynamic_pe_mode if hasattr(config, 'dynamic_pe_mode') else DynamicPeMode.N
        dynamic_pe_start_layer = config.dynamic_pe_start_layer if hasattr(config, 'dynamic_pe_start_layer') else 0

        dynamic_pe_layer_ids = (
            [int(idx) for idx in config.dynamic_pe_layer_ids.split(",")]
            if hasattr(config, "dynamic_pe_layer_ids") and config.dynamic_pe_layer_ids
            else [layer_id]
        )
        self.dynamic_pe_mode = dynamic_pe_mode if dynamic_pe_start_layer <= layer_id and layer_id in dynamic_pe_layer_ids else DynamicPeMode.N
        print(f"{layer_id}: {self.dynamic_pe_mode}")

        if self.dynamic_pe_mode == DynamicPeMode.COPE:
            head_dim = config.d_model // config.n_heads
            self.max_cope_length = min(config.max_sequence_length, 4096)
            self.dynamic_map = nn.Linear(head_dim, self.max_cope_length, bias=False)
        
        # Rotary embeddings.
        if self.config.rope:
            self.rotary_emb = RotaryEmbedding(config, self.__cache)
            self.pe_gen_func = self.rotary_emb.from_positions
    ...
    def set_activation_checkpointing(
        ...
        if strategy == ActivationCheckpointingStrategy.fine_grained or self.dynamic_pe_mode == DynamicPeMode.COPE:
    ...
    @torch.compile
    def cope(self, q, attn_logits):
        # calculate cope
        gates = F.sigmoid(attn_logits) # [B, H, N_q, N_k]
        pos = gates.flip(-1).cumsum(dim=-1).flip(-1)
        pos = pos.clamp(max=self.max_cope_length-1)
        pos_ceil, pos_floor = pos.ceil().long(), pos.floor().long()
        logits_int = self.dynamic_map(q) # [B, H, N_q, self.max_cope_length]
        
        # [B, H, N_q, self.max_cope_length] -> [B, H, N_q, N_k]
        logits_ceil, logits_floor = logits_int.gather(-1, pos_ceil), logits_int.gather(-1, pos_floor)
        w = pos - pos_floor
        return logits_ceil * w + logits_floor * (1-w)

    @torch.compile
    def cope_attn(self, q, k, v, attn_mask, dropout_p, **kwargs):
        q_len, k_len = q.size(-2), k.size(-2)
        scale = 1 / v.size(-1) ** 0.5
        attn_logits = torch.matmul(q, k.transpose(-1, -2)) * scale # [B, H, N_q, N_k] = [B, H, N_q, D_h] x [B, H, D_h, N_k]

        # causal mask
        attn_bias = torch.zeros(q_len, k_len, dtype=q.dtype, device=q.device)
        if attn_mask is None:
            temp_mask = torch.ones(q_len, k_len, dtype=torch.bool, device=q.device).tril(diagonal=0)
            attn_bias.masked_fill_(temp_mask.logical_not(), torch.finfo(v.dtype).min)
            attn_bias = attn_bias.to(q.dtype)
        else:
            if attn_mask.dtype == torch.bool:
                attn_bias.masked_fill_(attn_mask.logical_not(), torch.finfo(v.dtype).min)
            else:
                attn_bias = attn_mask + attn_bias

        attn_logits = attn_logits + attn_bias.unsqueeze(0).unsqueeze(0)
        
        # apply cope
        attn_logits = attn_logits + self.cope(q, attn_logits)
        attn_weights = F.dropout(nn.functional.softmax(attn_logits, dim=-1), p=dropout_p)
    ...
    def attention(
        ...
        if self.config.rope:
            ...
            q, k = self.rotary_emb(q, k, pos_cos, pos_sin)
        ...
        # shape: (B, nh, T, hs)
        if self.dynamic_pe_mode == DynamicPeMode.COPE:
            assert self._activation_checkpoint_fn is not None
            att = self._activation_checkpoint_fn(
                self.cope_attn,
                q,
                k,
                v,
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if self.dynamic_pe_mode in [DynamicPeMode.C, DynamicPeMode.CWSC]:
            self.dynamic_map = nn.Linear(config.d_model, 1, bias=False)
        elif self.dynamic_pe_mode in [DynamicPeMode.MHC]:
            self.dynamic_map = nn.Linear(config.d_model, config.n_heads, bias=False)
        elif self.dynamic_pe_mode in DynamicPeMode.SEXMH:
            mid_size = config.d_model // 8
            self.gate_map = nn.Linear(config.d_model, mid_size, bias=False)
            self.content_map = nn.Linear(config.d_model, mid_size, bias=False)
            self.final_map = nn.Linear(mid_size, config.n_heads, bias=False)
    ...
    def reset_parameters(self):
        ...
        if self.dynamic_pe_mode in [DynamicPeMode.C, DynamicPeMode.CWSC]:
            init_normal(self.dynamic_map, std, cutoff_factor)
        elif self.dynamic_pe_mode in [DynamicPeMode.MHC]:
            init_normal(self.dynamic_map, std, cutoff_factor)
        elif self.dynamic_pe_mode in DynamicPeMode.SEXMH:
            init_normal(self.gate_map, std, cutoff_factor)
            init_normal(self.content_map, std, cutoff_factor)
            init_normal(self.final_map, std, cutoff_factor)
    ...
    def forward(
        ...
        # config for dynamic pe
        # NOTE: here we only consider the training part without handling the kv-cache for some
        # specific methods, e.g., DynamicPeMode.SEXFEX
        assert layer_past is None
        hidden_states = h
        past_len = 0 if layer_past is None else layer_past[0].shape[-2]
        position_ids = torch.arange(past_len, past_len + h.shape[1]).to(h.device).unsqueeze(0)

        if self.dynamic_pe_mode in [DynamicPeMode.NOPE, DynamicPeMode.COPE]:
            pred_indices = torch.zeros_like(hidden_states)
            cos, sin = None, None
        else:
            if self.dynamic_pe_mode == DynamicPeMode.C:
                ...
                pred_indices, position_embeddings = self.dynamic_position_encoding_multihead_continuous(hidden_states, position_ids)
            elif self.dynamic_pe_mode == DynamicPeMode.SEXMH:
                pred_indices, position_embeddings = self.dynamic_position_encoding_swigluex_multihead(hidden_states, position_ids)
            else:
                pred_indices = position_ids
                pred_indices = pred_indices.repeat(hidden_states.shape[0], 1)
                position_embeddings = self.pe_gen_func(pred_indices)
    ...
    def dynamic_position_encoding_swigluex_multihead(self, hidden_states, position_ids):
        ...
        # [B, N, D] -> [B, N, H] -> [B, H, N]
        pred_indices = self.final_map(F.silu(self.gate_map(hidden_states)) * self.content_map(hidden_states)).transpose(-1, -2)
        B, H, N = pred_indices.shape
        pred_indices = pred_indices.reshape(B, H*N)
        cos, sin = self.pe_gen_func(pred_indices)
        cos = cos.reshape(B, H, N, -1)
        sin = sin.reshape(B, H, N, -1)
        return pred_indices, (cos, sin)
    ...
    def dynamic_position_encoding_multihead_continuous(self, hidden_states, position_ids):
        device, dtype = hidden_states.device, hidden_states.dtype
        pred_indices = self.dynamic_map(hidden_states).transpose(-1, -2) # [B, N, H] -> [B, H, N]
        B, H, N = pred_indices.shape
        pred_indices = pred_indices.reshape(B, H*N)
        cos, sin = self.pe_gen_func(pred_indices)
        cos = cos.reshape(B, H, N, -1)
