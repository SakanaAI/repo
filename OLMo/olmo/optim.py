    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith("bias"):
                ...
                decay.add(fpn)
            elif pn.endswith("weight") and isinstance(m, nn.Conv1d):
                decay.add(fpn)
...
def fix_optim_state_dict(optimizer: Optimizer, state_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for group, sd_group, osd_group in zip(optimizer.param_groups, state_dict["param_groups"], optim_state_dict["param_groups"]):
        sd_group['params'] = osd_group['params']
        for key in PARAM_GROUP_FIELDS:
