...
def greedy_search(model,
                  ...
                  output_pred_indices: bool = False) -> torch.LongTensor:
    if device is None:
    ...
    with torch.no_grad():
        for step in range(max_length - cur_len):
            ...
            outputs = model(**model_inputs, return_dict=True, output_pred_indices=True)
            next_token_logits = outputs.logits[:, -1, :]
            if pred_indices is None:
                pred_indices = outputs.pred_indices
            else:
                pred_indices = torch.cat([pred_indices, outputs.pred_indices], dim=-1)
