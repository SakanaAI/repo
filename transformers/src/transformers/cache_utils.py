...
class DynamicCache(Cache):
    ...
    def __init__(self, _distributed_cache_data: Optional[Iterable] = None) -> None:
        ...
        self.pe_cache: List[torch.Tensor] = []
    ...
    def update_pe(self, pe, layer_idx, kernel_size):
        # Update the cache
        if pe is not None:
            if len(self.pe_cache) <= layer_idx:
                for _ in range(len(self.key_cache), layer_idx):
                    self.pe_cache.append(torch.tensor([]))
                self.pe_cache.append(pe)
            elif (
                not self.pe_cache[layer_idx].numel()  # prefers not t.numel() to len(t) == 0 to export the model
            ):  # fills previously skipped layers; checking for tensor causes errors
                self.pe_cache[layer_idx] = pe
            else:
                self.pe_cache[layer_idx] =  torch.cat([self.pe_cache[layer_idx], pe], dim=-1)

        return self.pe_cache[layer_idx][-kernel_size:]
