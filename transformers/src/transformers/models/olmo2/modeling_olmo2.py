...
from .configuration_olmo2 import Olmo2Config, PEConfig
...
class Olmo2Attention(nn.Module):
    ...
    def __init__(
        ...
        pe_gen_func: Optional[Callable] = None,
        dynamic_pe_mode: Optional[DynamicPeMode] = DynamicPeMode.N,
    ):
        super().__init__()
        self.config = config
        ...
        if self.dynamic_pe_mode in [DynamicPeMode.C, DynamicPeMode.CWSC]:
            ...
            self.dynamic_map = nn.Linear(config.hidden_size, config.num_attention_heads, bias=False)
        elif self.dynamic_pe_mode in DynamicPeMode.SEXMH:
            mid_size = config.hidden_size // 8
            self.gate_map = nn.Linear(config.hidden_size, mid_size, bias=False)
            self.content_map = nn.Linear(config.hidden_size, mid_size, bias=False)
            self.final_map = nn.Linear(mid_size, config.num_attention_heads, bias=False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        input_shape = hidden_states.shape[:-1] # [B, N, D]
        hidden_shape = (*input_shape, -1, self.head_dim) # [B, N, H, D]

        query_states = self.q_norm(self.q_proj(hidden_states))
        key_states = self.k_norm(self.k_proj(hidden_states))
        value_states = self.v_proj(hidden_states)

        query_states = query_states.view(hidden_shape).transpose(1, 2) # [B, H, N, D]
        key_states = key_states.view(hidden_shape).transpose(1, 2) # [B, H, N, D]
        value_states = value_states.view(hidden_shape).transpose(1, 2) # [B, H, N, D]

        # get dynamic pe arguments
        dynamic_pe_pdrop = kwargs.get("dynamic_pe_pdrop", 0.0)
        position_ids = kwargs.get("position_ids", None)
        if self.dynamic_pe_mode == DynamicPeMode.NOPE:
            pred_indices = torch.zeros_like(hidden_states)
        else:
            if self.dynamic_pe_mode == DynamicPeMode.C:
                pred_indices, position_embeddings = self.dynamic_position_encoding_continuous(hidden_states, position_ids, dynamic_pe_pdrop=dynamic_pe_pdrop)
            elif self.dynamic_pe_mode == DynamicPeMode.MHC:
                pred_indices, position_embeddings = self.dynamic_position_encoding_multihead_continuous(hidden_states, position_ids, dynamic_pe_pdrop=dynamic_pe_pdrop)
            elif self.dynamic_pe_mode == DynamicPeMode.SEXMH:
                pred_indices, position_embeddings = self.dynamic_position_encoding_swigluex_multihead(hidden_states, position_ids, dynamic_pe_pdrop=dynamic_pe_pdrop)
            else:
        ...
        if self.config._attn_implementation != "eager":
            if (
                ...
                and self.dynamic_pe_mode != DynamicPeMode.A
            ):
                logger.warning_once(
    ...
    def dynamic_position_encoding_swigluex_multihead(self, hidden_states, position_ids, dynamic_pe_pdrop=0.0):
        ...
        if dynamic_pe_pdrop is not None:
            pred_indices = (1-dynamic_pe_pdrop) * pred_indices + dynamic_pe_pdrop * position_ids.unsqueeze(1)
    ...
    def dynamic_position_encoding_continuous(self, hidden_states, position_ids, dynamic_pe_pdrop=0.0):
        device, dtype = hidden_states.device, hidden_states.dtype
        pred_indices = self.dynamic_map(hidden_states).squeeze(-1)
        if dynamic_pe_pdrop is not None:
            pred_indices = (1-dynamic_pe_pdrop) * pred_indices + dynamic_pe_pdrop * position_ids
        pe = self.pe_gen_func(pred_indices)
        return pred_indices, pe

    def dynamic_position_encoding_multihead_continuous(self, hidden_states, position_ids, dynamic_pe_pdrop=0.0):
        device, dtype = hidden_states.device, hidden_states.dtype
...
class Olmo2DecoderLayer(GradientCheckpointingLayer):
    def __init__(
        ...
        self.self_attn = Olmo2Attention(
            config=config, layer_idx=layer_idx, pe_gen_func=pe_gen_func, dynamic_pe_mode=dynamic_pe_mode
        )
...
class Olmo2Model(Olmo2PreTrainedModel):
    def __init__(self, config: Olmo2Config):
        ...
        if hasattr(self.config, "pe_config"):
            ...
            dynmaic_pe_layer_ids = (
                ...
                else [idx for idx in range(config.num_hidden_layers)]
            )
        ...
        self.layers = nn.ModuleList(
            [
                Olmo2DecoderLayer(
                    config,
                    layer_idx,
                    self.rotary_emb.from_positions,
                    dynamic_pe_mode=self.dynamic_pe_mode if layer_idx in dynamic_pe_layer_ids else DynamicPeMode.N,
                )
    ...
    def forward(
        ...
        dynamic_pe_pdrop: Optional[float] = 0.0,
