...
from oe_eval.data.wikitableqa import PROMPT, FEWSHOT_PROMPT, TABLE_TYPE
...
def create_core_wikitableqa_tasks() -> dict:
    tasks = dict()
    for task_name in TABLE_TYPE:
        tasks[f"wikitableqa_{task_name}"] = create_wikitableqa_task(task_name) 
    return tasks


def create_wikitableqa_task(task_name):
    class WikiTableQA(GenericWikiTableQA):
        TASK_CONFIG_DEFAULTS = get_dict_with_defaults(
            {
                "dataset_name": task_name,
            },
            GenericWikiTableQA.TASK_CONFIG_DEFAULTS,
        )
...
class GenericWikiTableQA(Task):
    VERSION = 0
    REQUEST_TYPE = RequestType.GENERATE_UNTIL
    TASK_CONFIG_DEFAULTS = {
        "dataset_path": "huayangli/wikitableqa",
        "dataset_name": "table",
    ...
    def _process_doc(self, doc):
        ...
        query = PROMPT.format(question=question, table=table)
